...
public class AllDemoZahlenbuttonsEntsrrukten extends GraphicsProgram {
	...
	void drawButtons(int x, int y, int sizeX, int sizeY) {
		...
		for (int breite = 0; breite < sizeX; breite++) {
			for (int hohe = 0; hohe < sizeY; hohe++) {
				...
				if (count>1000000)count=Math.log10(count);
//				if (count>1000000 || count<1000000|| (count>0&&count<0.000001)||(count<0&&count>0.000001))count=Math.log10(count);
				...
				tit.setColor(new Color(253,0,0));
...
//			if(ButtonCount==null)ButtonCount=new double[1][15];
	...
	public static void main(String[] args) {
		new AllDemoZahlenbuttonsEntsrrukten().start(args);
	}
}
