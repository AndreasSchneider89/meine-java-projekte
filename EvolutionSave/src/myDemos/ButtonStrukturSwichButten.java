...
public class ButtonStrukturSwichButten extends GraphicsProgram {
	...
	// Swichbuttenidee für Buttenleiste: Zwich zwichen Zahlen, Formen, Paint, Fight
	void drawButtons3D(int x, int y, int sizeX, int sizeY) {
...
//		for (int breite = 0; breite < sizeX; breite++) {
			for (int hohe = 0; hohe < sizeY; hohe++) {
				...
				feld.setFillColor(new Color(0, 0, 0));
				feld.setColor(new Color(0, 253, 0));
				add(feld,  100 + x, hohe * 100 + y);
				double count = ButtonCount[sizeX][hohe];
				String s = Double.toString(count);
				GLabel l = new GLabel(s);
				l.setFont(l.getFont().deriveFont(30.0f));
				l.setColor(new Color(0, 253, 0));
				add(l,  100 + x, hohe * 100 + y + 100);
				if (ButtonTitle[sizeX][hohe] == null)
					ButtonTitle[sizeX][hohe] = "";
				GLabel tit = new GLabel(ButtonTitle[sizeX][hohe]);
				tit.setFont(l.getFont().deriveFont(30.0f));
				tit.setColor(new Color(0, 253, 0));
				add(tit, sizeX * 100 + x, hohe * 100 + y + 40);
...
//			if(ButtonCount==null)ButtonCount=new double[1][15];
	...
	public static void main(String[] args) {
		new ButtonStrukturSwichButten().start(args);
	}
}
